# Graph Report - agent-orchestration  (2026-06-29)

## Corpus Check
- 178 files · ~133,858 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2392 nodes · 3469 edges · 184 communities (149 shown, 35 thin omitted)
- Extraction: 81% EXTRACTED · 19% INFERRED · 0% AMBIGUOUS · INFERRED: 672 edges (avg confidence: 0.77)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `6d7c3f38`
...
Cohesion: 0.05
Nodes (49): Agent, TickReport, BarStep, ExitResult, Adjudicator, _cost_usd(), LlmClient, MockClient (+41 more)
...
Cohesion: 0.11
Nodes (28): clamp_extended_tp(), clamp_tightened_stop(), Clamp an observation's proposed stop so it can only tighten, never widen risk., Clamp a proposed take-profit so the final target can only extend outward., canned_adjudication(), canned_observation(), Deterministic mock LLM outputs.  The mock turns a structured envelope into a val, The control-group judgement: no adjustment, no veto (``confidence_delta = 0``). (+20 more)
...
Cohesion: 0.15
Nodes (26): build_exhaustion_context(), build_planner_context(), build_structure_context(), build_volume_context(), _distance_atr(), _empty_memory_summary(), _f(), _htf_rsi_state() (+18 more)
...
Cohesion: 0.11
Nodes (15): LiveContext, Live testnet execution context — the mirror-model bridge.  When ``--live-execute, Close the FULL remaining exchange position (reduce-only), querying live size., Place a STOP_MARKET (SL) + TAKE_PROFIT_MARKET (TP), both closePosition=true., Cancel the existing SL and place a new STOP_MARKET at ``new_stop`` (trail / brea, Cancel any remaining SL/TP for a trade (on close)., Close the FULL remaining exchange position (reduce-only), querying live size., Holds the testnet client + order log for one live run. Methods mirror executor s (+7 more)
...
Cohesion: 0.12
Nodes (20): atr(), obv(), Average True Range (pandas-ta)., Relative Strength Index (pandas-ta)., rsi(), df200(), _make_df(), Tests for pure indicator functions on synthetic 200-bar series. (+12 more)
...
Cohesion: 0.11
Nodes (19): _merge_state(), open_paper_trade(), Paper executor — records simulated trades. NEVER places a live order.  This is t, Persist updated exit state into ``trade_metadata`` (preserving other keys)., Bank a scaled-out leg: accrue realized pnl into ``trade_metadata``, stay open., Persist updated exit state into ``trade_metadata`` (preserving other keys)., Persist updated exit state into ``trade_metadata`` (preserving other keys)., Bank a scaled-out leg: accrue realized pnl into ``trade_metadata``, stay open. (+11 more)
...
Cohesion: 0.16
Nodes (14): open_trades_for(), The single trade-close path + the open-trades query.  Every exit route (stop/tar, evaluate_invalidation(), handle_invalidation(), Deterministic plan/setup invalidation.  Invalidation is evaluated by CODE, never, Return the highest-severity triggered level, or None.      A rule with ``on_clos, Evaluate invalidation across the plan's setups. Returns True if plan was killed., Tests for deterministic plan/setup invalidation. (+6 more)
...
Cohesion: 0.2
Nodes (13): _guard_live(), Apply the profile, then refuse to run live execution unless it is safe + determi, _client_with_filters(), Tests for the Binance testnet live-execution layer (pure logic + CLI guards).  N, Guard tests mutate the global settings singleton; snapshot + restore so test, _reset_settings(), _restore_settings(), test_guard_passes_with_safe_config() (+5 more)

### Community 55 - "Community 55"
Cohesion: 0.17
Nodes (15): _accumulate(), _ensure_plan(), Engine orchestration: replay, single-tick, and live-poll loops.  All three share, Walk historical feature rows, running the full loop on each as if live.      ``r, Single evaluation against the most recent feature row., Single evaluation against the most recent feature row., Poll loop: optionally refresh data, then run a tick, sleeping ``interval`` secon, Poll loop: optionally refresh data, then run a tick, sleeping ``interval`` secon (+7 more)
...
Cohesion: 0.15
Nodes (11): create(), OrderError, OrderResult, Binance Futures TESTNET client — the ONLY component that places real orders.  Th, Round ``qty`` DOWN to the symbol's stepSize. Returns 0.0 if below minQty., Round ``qty`` DOWN to the symbol's stepSize. Returns 0.0 if below minQty., Place a MARKET order. ``side`` is 'BUY'/'SELL'. Returns the normalized fill., Place a MARKET order. ``side`` is 'BUY'/'SELL'. Returns the normalized fill. (+3 more)

### Community 61 - "Community 61"
Cohesion: 0.16
Nodes (6): BinanceFuturesTestnet, Return the open position for ``symbol`` (positionAmt != 0), else None., Return the open position for ``symbol`` (positionAmt != 0), else None., Place a CONDITIONAL algo order (STOP_MARKET / TAKE_PROFIT_MARKET / TRAILING_STOP, Cancel a single conditional algo order. Non-fatal: a gone order is logged, not r, Signed testnet client. Construct via ``await BinanceFuturesTestnet.create(...)``
...
Cohesion: 0.19
Nodes (12): Live position/order tracker — keeps internal state honest against the exchange., Reconcile internal open trades vs the live testnet position for ``symbol``., Log a one-line live position snapshot (qty / entry / unrealized PnL)., reconcile_on_start(), snapshot(), _consume_klines(), _consume_user(), Websocket-driven live loop (event-driven, bar-close-precise).  A quick, robust i (+4 more)

### Community 65 - "Community 65"
Cohesion: 0.18
Nodes (12): Render a fingerprint as a pgvector text literal, e.g. ``[0.1,0.2,...]``., to_vector_literal(), _avg(), Retrieve the most-similar prior learnings for a setup (cosine over the fingerpri, Top-k learnings nearest to ``fingerprint`` by cosine distance.      ``direction`, Aggregate nearby learning rows into a compact planner-facing memory signal., retrieve_memory_summary(), retrieve_relevant_learnings() (+4 more)
...
Cohesion: 0.15
Nodes (12): AI Crypto Trader Architecture Summary, code:text (AI decides WHAT to trade), code:text (Market Stream (WebSocket)), code:text (create_plan     = strategist), code:text (LLM defines the trading plan.), code:json ({), Core Principle, Feature Builder (+4 more)
...
Cohesion: 0.2
Nodes (10): Entry path: size and open a detected setup deterministically.  The per-setup LLM, setup_dict(), _arm_limit_price(), _entry_gates_ok(), evaluate_now(), Orchestrator — wires the engine collaborators for one evaluation tick.  For a gi, Run one full evaluation of the active plan against the current feature row., Run one full evaluation of the active plan against the current feature row. (+2 more)
...
Cohesion: 0.27
Nodes (10): execute_setup(), Size and open a detected setup deterministically at ``entry_price``.      ``entr, Size and open a detected setup deterministically at ``entry_price``.      ``entr, exit_policy_for_regime(), exit_policy_from_metadata(), exit_policy_metadata(), ExitPolicy, is_sideways() (+2 more)
...
Cohesion: 0.33
Nodes (4): _FakeRaw, Captures kwargs passed to futures_create_algo_order., test_place_conditional_stop_market_closeposition(), test_place_conditional_trailing_uses_callback_and_quantity()

### Community 106 - "Community 106"
Cohesion: 0.4
Nodes (5): advance_trade(), Deterministic exit machine driver.  Walks each open trade across a sequence of c, Advance each open trade. Uses ``fine_candles`` (finer-tf) when given, else this, Walk one open trade across a sequence of candles (finer-tf bars or a single bar), reconcile_open_trades()
...
Cohesion: 0.5
Nodes (4): _outcome(), Post-mortem: on a closed trade, write one structured ``learning`` row.  Called f, Run the post-mortem for ``trade`` and persist a learning + audit row., reflect_and_store()
...
Cohesion: 0.4
Nodes (4): Fetch and cache LOT_SIZE / PRICE_FILTER / MIN_NOTIONAL for ``symbol``., Trading filters for one symbol, used to round quantities to an acceptable order., Fetch and cache LOT_SIZE / PRICE_FILTER / MIN_NOTIONAL for ``symbol``., SymbolFilters
...
## Knowledge Gaps
- **1004 isolated node(s):** `Mean-reversion proposer: range-fade triggers, skips, RR floor, causality + route`, `20 range bars (high 110 / low 90) + one wild decision bar that must be EXCLUDED.`, `Tests for momentum_composite with pinned MACD_SCALE golden values.`, `MACD_SCALE must be pinned at 1.0 (golden value freeze).`, `rsi=30 → rsi_n = 0.0; with macd_hist=0 → macd_n=0.5; roc_5=0 → roc_n=0.5.      r` (+999 more)
...
- **Why does `timeframe_to_timedelta()` connect `Community 76` to `Community 65`, `Community 6`, `Community 10`, `Community 77`, `Community 46`, `Community 55`?**
  _High betweenness centrality (0.078) - this node is a cross-community bridge._
- **Why does `_run()` connect `Community 59` to `Community 0`, `Community 66`, `Community 104`, `Community 43`, `Community 44`, `Community 46`, `Community 47`, `Community 81`, `Community 53`, `Community 29`?**
  _High betweenness centrality (0.076) - this node is a cross-community bridge._
- **Why does `_ensure_data()` connect `Community 10` to `Community 42`, `Community 43`, `Community 76`?**
