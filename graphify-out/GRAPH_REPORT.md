...
## Corpus Check
- 178 files · ~126,246 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2121 nodes · 3113 edges · 163 communities (134 shown, 29 thin omitted)
- Extraction: 80% EXTRACTED · 20% INFERRED · 0% AMBIGUOUS · INFERRED: 630 edges (avg confidence: 0.77)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9bdea9d6`
...
Cohesion: 0.07
Nodes (49): Agent, TickReport, Adjudicator, _cost_usd(), LlmClient, MockClient, Observer, OpenAIClient (+41 more)
...
Cohesion: 0.07
Nodes (44): direction_vote(), Direction vote + weighted statistics over the eight agent scores.  Abstaining ag, Return (long_pressure, short_pressure) = Σ weight·score on each side., Weighted majority direction, or ``neutral`` on a tie / all-neutral., Weighted mean of scores from agents voting the chosen ``direction``.      Normal, Variance of the agreeing agents' scores (drives the alignment penalty).      Dis, score_variance(), weighted_mean() (+36 more)
...
Cohesion: 0.08
Nodes (40): Base, Basis, Candle, Feature, FundingRate, FundingRateXVenue, Heartbeat, Liquidation (+32 more)
...
Cohesion: 0.07
Nodes (29): Accept / reject, Accept / reject → REJECTED, Attempt 1 — `reward_atr_mult 2.0 → 1.2` (fire the scale-out on more winners) → REJECTED, Attempt 2 — `signal_min_confidence 0.70 → 0.75` (filter marginal setups) → REJECTED (overfit), Holdout NOT run — stop condition never met (no accepted change)., Honest read of the generalization gap, Honest read of the generalization gap, Honesty caveat discovered this iteration (+21 more)
...
Cohesion: 0.16
Nodes (16): FakeSession, ObserveClient, _patch_detector(), Detector entry-loop tests., _setup(), test_armed_setup_fills_on_a_later_touch_at_limit_price(), test_entry_confirmation_allows_confirmed_setup(), test_entry_confirmation_blocks_unconfirmed_setup() (+8 more)
...
Cohesion: 0.11
Nodes (18): AI Crypto Trader Architecture Summary, code:text (AI decides WHAT to trade), code:json ({), code:python (if setup.plan_id != current_plan.plan_id:), code:text (Market Stream (WebSocket)), code:text (create_plan), code:text (confirm_setup), code:text (create_plan     = strategist) (+10 more)
...
Cohesion: 0.15
Nodes (15): close_paper_trade(), margin_close_values(), _merge_state(), Paper executor — records simulated trades. NEVER places a live order.  This is t, Persist updated exit state into ``trade_metadata`` (preserving other keys)., Persist a no-fill state change (e.g. a trailing-stop move) for an open trade., Mark a trade closed and record realized pnl.      ``exit_result.pnl_pct`` is the, Identifies the replay run that opened a trade (for the A/B harness). (+7 more)
...
Cohesion: 0.17
Nodes (14): invalidate_check(), _parse_dt(), CLI: ats engine — run the trading loop (replay / tick / live)., Replay the full loop over historical features (the primary POC demo)., Parse an ISO date ('2026-05-01') or datetime into a UTC-aware datetime., Single evaluation against the latest computed features., Evaluate the active plan's invalidation rules against the latest closed bar., Live poll loop: (optionally refresh data,) then tick, repeating every --interval (+6 more)
...
Cohesion: 0.21
Nodes (12): _agent_input(), propose_plan(), propose_signal(), Deterministic proposer — run the 8 agents + synthesizer over a plan envelope.  T, Run the 8 agents + synthesizer over the envelope and apply the hard regime gate., Run the deterministic signal engine and bridge it to a PlanOutput (control group, _breakout_envelope(), _candle() (+4 more)
...
Cohesion: 0.21
Nodes (13): _accumulate(), _ensure_plan(), Engine orchestration: replay, single-tick, and live-poll loops.  All three share, Walk historical feature rows, running the full loop on each as if live.      ``r, Single evaluation against the most recent feature row., Poll loop: optionally refresh data, then run a tick, sleeping ``interval`` secon, Append this tick's notes, collapsing sticky states to start/resume transitions., Refresh the plan if there is none, it expired, or the refresh cadence elapsed. (+5 more)
...
Cohesion: 0.25
Nodes (9): Settings, apply_profile(), Named strategy profiles — bundles of engine/risk knobs applied as a unit.  A pro, Apply a named profile's grouped overrides onto ``target`` (defaults to the singl, BaseSettings, Tests for named strategy profiles., test_scalper_profile_sets_strategy_metadata(), test_scalper_profiles_enable_time_stop() (+1 more)
...
Cohesion: 0.25
Nodes (7): execute_setup(), Entry path: size and open a detected setup deterministically.  The per-setup LLM, Size and open a detected setup deterministically at ``entry_price``.      ``entr, setup_dict(), open_paper_trade(), Insert an open paper trade for a detected+confirmed+risk-approved setup.      ``, exit_policy_metadata()

### Community 80 - "Community 80"
Cohesion: 0.32
Nodes (7): _arm_limit_price(), _entry_gates_ok(), evaluate_now(), Orchestrator — wires the engine collaborators for one evaluation tick.  For a gi, Run one full evaluation of the active plan against the current feature row., The resting-limit price for a setup: the zone edge a fill is *worst* at.      Lo, Non-price entry gates: regime-direction filter + optional bar-close confirmation

### Community 81 - "Community 81"
Cohesion: 0.29
Nodes (7): Bank a scaled-out leg: accrue realized pnl into ``trade_metadata``, stay open., record_partial_exit(), advance_trade(), Deterministic exit machine driver.  Walks each open trade across a sequence of c, Advance each open trade. Uses ``fine_candles`` (finer-tf) when given, else this, Walk one open trade across a sequence of candles (finer-tf bars or a single bar), reconcile_open_trades()
...
Cohesion: 0.33
Nodes (6): build_envelope(), _prune_missing(), Drop feature keys whose value is None or NaN before sending to the strategist., Assemble the JSON-only context passed to create_plan., test_prune_missing_drops_none_and_nan(), test_build_envelope_includes_planner_context_and_prior_lessons()
...
## Knowledge Gaps
- **448 isolated node(s):** `list`, `symbol`, `markPrice`, `indexPrice`, `lastFundingRate` (+443 more)
...
- **Why does `_run()` connect `Community 55` to `Community 1`, `Community 40`, `Community 42`, `Community 75`, `Community 43`, `Community 50`, `Community 51`, `Community 26`, `Community 93`, `Community 62`?**
  _High betweenness centrality (0.115) - this node is a cross-community bridge._
- **Why does `timeframe_to_timedelta()` connect `Community 67` to `Community 6`, `Community 8`, `Community 43`, `Community 79`, `Community 60`, `Community 94`?**
  _High betweenness centrality (0.074) - this node is a cross-community bridge._
- **Why does `_ensure_data()` connect `Community 8` to `Community 41`, `Community 67`, `Community 51`?**
