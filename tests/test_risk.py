...
def test_sizing_targets_risk_budget_when_uncapped() -> None:
    # stop 5% away, 1% risk of $10k → $100 at risk; notional = 100/0.05 = $2000 → 0.2x.
    ...
    assert s.leverage == pytest.approx(0.2)
    assert s.liq_price is None  # below 1x: no liquidation risk


def test_sizing_capped_by_leverage_lowers_realized_risk() -> None:
    # Tight 0.1% stop wants huge notional; max_leverage caps it → realized risk < budget.
    s = size_for_risk("long", 100, 99.9, equity_usd=EQUITY, risk_pct=0.01, max_leverage=3.0)
    assert s.ok
    assert s.leverage == pytest.approx(3.0)
    assert s.notional_usd == pytest.approx(30_000.0)
    assert s.risk_usd == pytest.approx(30.0)  # 30000 * 0.001, well under the $100 budget
...
def test_sizing_rejects_stop_beyond_liquidation() -> None:
    # High leverage + a stop wider than the liquidation distance → rejected.
    s = size_for_risk("long", 100, 98, equity_usd=EQUITY, risk_pct=0.95, max_leverage=50.0)
    assert not s.ok
    assert "liquidation" in s.reason
...
def test_approved_sizes_by_risk() -> None:
    ...
    assert d.leverage == pytest.approx(0.2)
